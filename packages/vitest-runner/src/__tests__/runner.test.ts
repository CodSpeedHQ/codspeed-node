...
describe("CodSpeedRunner", () => {
  it("should run the bench functions only twice", async () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      "[CodSpeed] running suite packages/vitest-runner/src/__tests__/runner.test.ts"
    );

    // run
    expect(benchFn).toHaveBeenCalledTimes(2);
    expect(console.log).toHaveBeenCalledWith(
      "[CodSpeed] packages/vitest-runner/src/__tests__/runner.test.ts::test bench done"
    );

    // teardown
    expect(console.log).toHaveBeenCalledWith(
      "[CodSpeed] running suite packages/vitest-runner/src/__tests__/runner.test.ts done"
    );
  ...
  it("should run nested suites", async () => {
    ...
    expect(benchFn).toHaveBeenCalledTimes(2);
    expect(console.log).toHaveBeenCalledWith(
      "[CodSpeed] packages/vitest-runner/src/__tests__/runner.test.ts::nested suite::test bench done"
    );
